...
load("lnfsForagingTrips_ShelfVars(iso2000).Rdata")
ft <- tbl_df(ft)
...
ftl2 %>% 
  ...
  geom_point( aes(color = floc), size = 0.8) + 
  facet_wrap(year~id) +
  geom_text(aes(label = switch), size = 3, alpha = 0.6) +
  labs(x = 'Date')
```
...
load('ASLCTD_IMOS_2016-2017.RData')
imos2 <- imos2 %>% mutate(year = year(date))

## load IMOS KI Mooring DATA
ki <- read_csv("IMOS_-_Australian_National_Mooring_Network_(ANMN)_Facility_-_Temperature_and_salinity_time-series.csv",
              skip = 22)
...
w %>% filter(year < 2018) %>% group_by(year, season) %>% summarise(mean = mean(wind_index), sd = sd(wind_index)) 
  # ggplot(aes(season, mean)) + 
  # geom_col(position = 'dodge') + 
  # facet_wrap(~year)
w %>% filter(year < 2018) %>% ggplot(aes(season, wind_index)) + geom_boxplot() + facet_wrap(~year)

a <- w$wind_index[w$year == 2016 & w$season == 'upwelling']
b <- w$wind_index[w$year == 2017 & w$season == 'upwelling']
wilcox.test(a,b)
a <- w$wind_index[w$year == 2016 & w$season == 'downwelling']
b <- w$wind_index[w$year == 2017 & w$season == 'downwelling']
wilcox.test(a,b)
a <- w$wind_index[w$year == 2016]
b <- w$wind_index[w$year == 2017]
wilcox.test(a,b)
...
#remove x lab
rmxlab <- theme(axis.title.x = element_blank())
...
aimos %>% 
  ...
  geom_rect(data = fts2, aes(xmin = xmin, xmax = xmax, ymin = 11.5, ymax = 20 ), alpha = 0.2) +
  scale_x_date(date_breaks = '1 month', date_labels = '%b', limits = as.Date(c('2000-01-01', '2000-12-31'))) +
  scale_colour_manual(values = wesanderson::wes_palette(n=3, name="GrandBudapest"),
                      name = 'Shelf temperature', labels = c('maximum', 'mean','minimum')) +
   scale_linetype_manual(values = c('dotted', 'solid', 'dotted'), name = 'Mooring temperature', labels = c('maximum', 'mean','minimum')) +
  # labs(y = 'Temperature (°C)') +
  labs(y = 'Temperature') +
...
w %>%
  ...
  geom_rect(data = fts2, aes(xmin = xmin, xmax = xmax, ymin = -.189, ymax = .018 ), alpha = 0.2) +
...
png(filename = 'interannual_shelf2.png',  width=9, height=7, units= "in", res = 300)
...
ft3 <- ftl2 %>% dplyr::select(-X, -id, -minLat, -Lon_minLat, -floc, -date, -season, -year, -switch,
                              -SSTA_min, -SSHA_max, -SSHA_mean, -SSTA_max,-t_max,-t_SD, -t_min )
                               # -SSTA_min, -SSHA_max, -SSHA_mean, -SSTA_max, -t_SD)
corvif(ft3)

# add the nominal variables back to processed dataset.
ft3 <- bind_cols(ft3, ftl2[,c('season', 'year', 'id', 'floc', 'X','date','switch')])
```
...
# ft5 <- ft4 %>% mutate(tmean = tmean, salmean = salmean, mldmean = mldmean,
#                       sdSSTa = log(sdSSTa))
ft4 <- ft3
...
```{r}
mod00 <- glm(floc ~ t_mean + sal_mean + SSHA_min + wind_index + year , data = data2, family = binomial)
mod1 <- glmer(floc ~ t_mean + sal_mean + SSHA_min + wind_index + year + (1 | id), data = data2, family = binomial)
anova(mod1, mod00)
```
  
Here I test trip nested within id as random effect
```{r}
mod00 <- glm(floc ~ tmean + salsd + salmean + mldsd + mldmean + sam + ASSTa + sdSSTa + maxSSTa + year , data = data2, family = binomial)
mod1 <- glmer(floc ~ tmean + salsd + salmean + mldsd + mldmean + sam + ASSTa + sdSSTa + maxSSTa + year + (1 | id/X), data = data2, family = binomial)
mod1
anova(mod1, mod00)
```
   Model fails to converge - probably because random effect is too complex?

```{r}
mod00 <- glm(floc ~ tmean + salsd + salmean + mldsd + mldmean + sam + ASSTa + sdSSTa + maxSSTa + year + X, data = data2, family = binomial)
mod1 <- glmer(floc ~ tmean + salsd + salmean + mldsd + mldmean + sam + ASSTa + sdSSTa + maxSSTa + year + X + (1 | id), data = data2, family = binomial)
# summary(mod1, corr = F)
mod1
anova(mod1, mod00)
```
...
mod0 <- glmer(floc ~ 1 + (1 | id), data = data2, family = binomial, nAGQ = 25)
mod1 <- glmer(floc ~ t_mean + sal_SD + sal_mean + mld_SD + mld_mean + sam + SSTa_ncells + SSTA_SD +
                SSTA_mean + SSHA_SD + SSHA_min + wind_index +  year + (1 | id), data = data2, family = binomial, nAGQ = 25)
drop1(mod1, test="Chisq")
mod2 <- update(mod1, .~. -mld_mean)
drop1(mod2,test="Chisq")
mod3 <- update(mod2, .~. -sal_SD)
drop1(mod3,test="Chisq")
mod3a <- update(mod3, .~. -wind_index)
drop1(mod3a,test="Chisq")
mod4 <- update(mod3a, .~. -SSTA_mean)
drop1(mod4,test="Chisq")
mod5 <- update(mod4, .~. -SSTA_SD)
drop1(mod5,test="Chisq")
mod6 <- update(mod5, .~. -sam)
drop1(mod6,test="Chisq")
mod7 <- update(mod6, .~. -year)
drop1(mod7,test="Chisq")
mod8 <- update(mod7, .~. -mld_SD)
drop1(mod8,test="Chisq")
mod9 <- update(mod8, .~. -SSHA_SD)
drop1(mod9,test="Chisq")
mod10 <- update(mod9, .~. -SSTa_ncells)
drop1(mod10,test="Chisq")
mod11 <- update(mod10, .~. -SSHA_min)
drop1(mod11,test="Chisq") 

# add plausible interactions with strongest x terms
mod12 <- update(mod11, .~. -t_mean + sal_mean*SSHA_min + SSTA_mean)
drop1(mod12, test = 'Chisq')
mod13 <- update(mod10, .~. +t_mean*SSHA_min)
drop1(mod13, test = 'Chisq')
mod14 <- update(mod10, .~. +sal_mean*SSHA_min)
drop1(mod14, test = 'Chisq')
mod15 <- update(mod10, .~. +t_mean*sal_mean)
drop1(mod15, test = 'Chisq')
mod16 <- update(mod10, .~. -t_mean -sal_mean)
drop1(mod16, test = 'Chisq')
mod17 <- update(mod14, .~. -t_mean)
drop1(mod17, test = 'Chisq')
```
...
aicc <- bbmle::ICtab(mod0,mod1,mod2,mod3,mod3a,mod4,mod5,mod6,mod7,mod8,mod9,mod10,mod11,mod12,mod13,mod14,mod15,mod16,mod17, base = TRUE, logLik = TRUE, weights = TRUE, type = "AICc")
aicc <- data.frame(aicc[], mod = attributes(aicc)$row.names)
mlist <- list(mod0,mod1,mod2,mod3,mod3a,mod4,mod5,mod6,mod7,mod8,mod9,mod10,mod11,mod12,mod13,mod14,mod15,mod16,mod17)
...
n <- 0:17
mn <- sprintf(paste0("mod", n), n)
aicc <- bbmle::ICtab(mod0,mod1,mod2,mod3,mod4,mod5,mod6,mod7,mod8,mod9,mod10,mod11,mod12,mod13,mod14,mod15,mod16,mod17, base = TRUE, logLik = TRUE, weights = TRUE, type = "AICc")
aicc <- data.frame(aicc[], mod = attributes(aicc)$row.names) %>% mutate_if(is.numeric, funs(signif(., 3) ))
mlist <- list(mod0,mod1,mod2,mod3,mod4,mod5,mod6,mod7,mod8,mod9,mod10,mod11,mod12,mod13,mod14,mod15,mod16,mod17)
formula <- map_chr(mlist, function(x)strsplit(as.character(attributes(x@frame)$formula), '~')[[3]])
mst <- data.frame(mod = mn, formula = formula)
mst <- left_join(mst, aicc, by = 'mod')
mst <- mst %>% arrange(dAICc)
mst[,-1] %>% 
  kable('html') %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), font_size = 12)
write.csv(mst[,-1], file = 'glmm selection results.csv')
...
p3 <- p2$plot.list[[1]] + 
  mytheme +
  labs(x = 'Average salinity (g/kg)', y = 'Probability of shelf foraging', title = NULL)
...
png(filename = 'glmm.png',  width=7, height=7, units= "in", res = 300)
